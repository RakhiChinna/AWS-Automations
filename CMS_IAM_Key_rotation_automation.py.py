...
def lambda_handler(event, context):
    kms_key_id = "arn:aws:kms:us-east-1:249864557561:key/ada685b7-8f76-41bd-bfe7-9de2946b4129"  # Replace with your KMS Key ARN
    ...
    if users:
        for user in users:
            ...
            # :white_check_mark: Filter only users with application tag = CMS (case-insensitive)
            if tag_dict.get('application', '').lower() != 'cms':
                logger.info(f"Skipping user {user_name}: application tag is not CMS")
                continue
            if 'SecretName' in tag_dict and 'AccessKeyName' in tag_dict and 'SecretKeyName' in tag_dict:
                ...
                try:
                    ...
                    if not current_keys:
                        ...
                        continue
                    current_key_metadata = current_keys[0]  # Assume one key
                    create_date = current_key_metadata['CreateDate']
                    if is_key_older_than_one_minute(create_date):
                        # Delete old access key
                        iam_client.delete_access_key(
                            UserName=user_name,
                            AccessKeyId=decrypt_accesskey_id['Plaintext'].decode('utf-8')
                        )
                        ...
                        # Encrypt with KMS and base64 encode
